<template>
    <div class="flex flex-col flex-1 items-center">
        <div v-if="route.query.preview" class="text-white p-4 bg-weather-secondary w-full text-center">
            <p>You arre currently previewing this city, click the "+" icon to start tracking this city.</p>
        </div>
        <div class="flex flex-col items-center text-white py-12">
            ...
            <p class="text-8xl mb-8">
                {{ convertToCelsius(weatherData.current.temp) }}°C
            </p>
            <div class="text-center">
                <p> Feels like {{ convertToCelsius(weatherData.current.feels_like) }}°C</p>
            ...
            <div class="mx-8 text-white">
                ...
                <div class="flex gap-10 overflow-x-scroll">
                    <div v-for="hourData in weatherData.hourly" :key="hourData.dt" class="flex flex-col gap-4 items-center">
                        ...
                        <p class="text-xl">{{ convertToCelsius(weatherData.current.temp) }}°C</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="mx-8 text-white">
                ...
                <div v-for="day in weatherData.daily" :key="day.dt" class="flex items-center">
                    ...
                    <div class="flex gap-2 flex-1 justify-end">
                        <p>H: {{ convertToCelsius(Math.round(day.temp.max)) }}</p>
                        <p>L: {{ convertToCelsius(Math.round(day.temp.min)) }}</p>
                    </div>
                </div>
            </div>
        </div>
